...
import { X, Plane, Utensils, Wifi, Armchair, Calendar, Percent, Image, PlusCircle, Trash2, Loader2, Globe } from 'lucide-react';

const EditFlightModal = ({ isOpen, onClose, onSuccess, flightData: initialFlightData }) => {
    ...
    // This effect runs when the modal is opened or the flightData prop changes.
    useEffect(() => {
        if (initialFlightData) {
            ...
            const data = {
                ...
                customFields: [],
                ...initialFlightData,
                // ✅ NEW: Determine flight type from existing data
                flightType: (initialFlightData.internationalGst || initialFlightData.economyInternationalGst) ? 'international' : 'domestic',
            };
            setFormData(data);
        }
    }, [initialFlightData]);
    ...
    const handleChange = (e) => {
        ...
        if (type === 'checkbox') {
            setFormData(prev => ({
                ...prev,
                inFlightServices: checked
                    ? [...(prev.inFlightServices || []), name]
                    : (prev.inFlightServices || []).filter(service => service !== name)
            }));
        } else {
            const isNumber = [
            ...
            setFormData(prev => {
                ...
                // When flight type changes, clear the irrelevant GST fields
                if (name === "flightType") {
                    if (value === 'domestic') {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${apiUrl}/api/admin/flights/${formData.id}`, {
                method: 'PUT',
            ...
            if (!response.ok) {
                ...
                throw new Error(data.message || 'Failed to update the flight route.');
            }
            onSuccess();
        } catch (err) {
                ...
                <motion.div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
                    <motion.div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden" initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 50 }}>
                        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex justify-between items-center p-5">
                            <h3 className="text-xl font-bold flex items-center gap-2"><Plane size={24} /> Edit Flight</h3>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <fieldset className="border rounded-lg p-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div><label className={labelStyle}>Airline Name</label><input type="text" name="airline" value={formData.airline || ''} onChange={handleChange} required className={inputStyle} /></div>
                                        <div><label className={labelStyle}>Flight Number</label><input type="text" name="flightNumber" value={formData.flightNumber || ''} onChange={handleChange} required className={inputStyle} /></div>
                                    </div>
                                </fieldset>
                                <fieldset className="border rounded-lg p-4">
                                    <legend className="text-sm font-semibold px-2">Route & Schedule</legend>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="md:col-span-2"><label className={`${labelStyle} flex items-center`}><Calendar size={16} className="mr-1"/> Date of Flight</label><input type="date" name="date" value={formData.date || ''} onChange={handleChange} required min={today} className={inputStyle} /></div>
                                        ...
                                        <div className="md:col-span-2"><label className={labelStyle}>Duration</label><input type="text" name="duration" value={formData.duration || ''} readOnly placeholder="Auto-calculated" className={`${inputStyle} bg-gray-100 cursor-not-allowed`} /></div>
                                    </div>
                                </fieldset>
                                <fieldset className="border rounded-lg p-4">
                                    <legend className="text-sm font-semibold px-2">Pricing & Taxes</legend>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div><label className={labelStyle}>Economy Price (₹)</label><input type="number" name="economyPrice" min="0" value={formData.economyPrice || ''} onChange={handleChange} required className={inputStyle} /></div>
                                        <div><label className={labelStyle}>Business Price (₹)</label><input type="number" name="businessPrice" min="0" value={formData.businessPrice || ''} onChange={handleChange} required className={inputStyle} /></div>
                                        <div className="md:col-span-2">
                                            <label className={`${labelStyle} flex items-center`}><Globe size={16} className="mr-1"/> Flight Type</label>
                                            <select name="flightType" value={formData.flightType || 'domestic'} onChange={handleChange} className={inputStyle}>
                                        ...
                                        {formData.flightType === 'domestic' && (
                                            <>
                                                <div><label className={`${labelStyle} flex items-center`}><Percent size={14} className="mr-1" /> Economy Domestic GST (%)</label><input type="number" name="economyDomesticGst" min="0" step="0.01" value={formData.economyDomesticGst || ''} onChange={handleChange} className={inputStyle} placeholder="e.g., 5" /></div>
                                                <div><label className={`${labelStyle} flex items-center`}><Percent size={14} className="mr-1" /> Business Domestic GST (%)</label><input type="number" name="businessDomesticGst" min="0" step="0.01" value={formData.businessDomesticGst || ''} onChange={handleChange} className={inputStyle} placeholder="e.g., 12" /></div>
                                            </>
                                        )}
                                        {formData.flightType === 'international' && (
                                            <>
                                                <div><label className={`${labelStyle} flex items-center`}><Percent size={14} className="mr-1" /> Economy Intl. GST (%)</label><input type="number" name="economyInternationalGst" min="0" step="0.01" value={formData.economyInternationalGst || ''} onChange={handleChange} className={inputStyle} placeholder="e.g., 18" /></div>
                                                <div><label className={`${labelStyle} flex items-center`}><Percent size={14} className="mr-1" /> Business Intl. GST (%)</label><input type="number" name="businessInternationalGst" min="0" step="0.01" value={formData.businessInternationalGst || ''} onChange={handleChange} className={inputStyle} placeholder="e.g., 18" /></div>
                                            </>
                                        )}
                                    </div>
                                </fieldset>
                                <fieldset className="border rounded-lg p-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div><label className={labelStyle}>Stops</label><input type="number" name="stops" min="0" value={formData.stops || 0} onChange={handleChange} required className={inputStyle} /></div>
                                    ...
                                    {(formData.stops || 0) > 0 && (
                                        <div className="mt-4 border-t pt-4">
                                            ...
                                            <div className="space-y-2">
                                                {(formData.stopLocations || []).map((stop, index) => (
                                                    <div key={index}>
                                ...
                                <fieldset className="border rounded-lg p-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <label className="flex items-center space-x-2"><input type="checkbox" name="In-flight Meal" checked={(formData.inFlightServices || []).includes('In-flight Meal')} onChange={handleChange} className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" /><Utensils size={16} /><span>Meal</span></label>
                                            <label className="flex items-center space-x-2"><input type="checkbox" name="WiFi Available" checked={(formData.inFlightServices || []).includes('WiFi Available')} onChange={handleChange} className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" /><Wifi size={16} /><span>WiFi</span></label>
                                            <label className="flex items-center space-x-2"><input type="checkbox" name="Extra Legroom" checked={(formData.inFlightServices || []).includes('Extra Legroom')} onChange={handleChange} className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" /><Armchair size={16} /><span>Extra Legroom</span></label>
                                        </div>
                                    </div>
                                    <div className="mt-4 border-t pt-4">
